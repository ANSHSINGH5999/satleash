...
Suite: **161 tests in 22 files, 161 pass** (`npm run check`); **46 of 46 e2e checks**; `npm audit` 0 vulnerabilities.
...
| Browser (Chrome) | Console tabs and keyboard, hostile strings as text, live updates, relay management, public relay test flow, error explanations, landing drill, landing hero and cursor lens, 4.5:1 contrast of the listed text colours and the hero button (fixed in the final QA, see `landing-design.md`; not a full accessibility audit), landmarks and labels | Works; nothing executes | 13 tests pass | PASS | `ui.test.ts` |
| LND (real) | Least-privilege macaroons, live channel update, monitor, lnd validates the relay copy, corrupted blob rejected, CLI, wipe and restore with the restricted macaroon | As specified | 46 of 46 | PASS | `npm run e2e` |
...
| Recovery (real) | Two-channel drill, verify, wipe, restore with one relay off, funds back | Funds return less fees | 1,492,866 of 1,493,060 sats, 194 sats fees, roughly 36 to 41 s across recorded runs | PASS | drill runs, e2e |
...
| Demo | Every command in the landing page's "Run it" section, run literally against real regtest LND: `npm install`, `web`, `playground`, `demo`, `bake` (and refusal to overwrite), `bake --restore`, `daemon` (published, stopped cleanly), `verify` (exit 0, verdict verified, lnd accepted the copy; exit 1 when the relays are down), `restore` on a wiped node recreated from the same seed | Each works as documented | All worked. `restore`: 2 channels awaiting force-close, then 1,492,866 of 1,493,060 sats recovered, same node identity. Found and fixed: `verify` and `pubkey` printed a log line on stdout ahead of their result; logs now go to stderr | PASS | manual runs, e2e (`cli verify prints pure JSON`, `cli pubkey prints exactly the 64-hex key`), `log.test.ts` |
| Demo | The landing page's own command strings, extracted from the HTML and run in zsh from the parent of the project folder (`cd lifeboat`, `playground`, `export LND_DIR=…`, `bake`, `daemon`) | Run as pasted | Worked once the lnd port and relays were pointed at the playground; `$LND_DIR` expands; before the fix `cd` was missing and `<lnd dir>` failed in zsh | PASS | manual run, `ui.test.ts` |
